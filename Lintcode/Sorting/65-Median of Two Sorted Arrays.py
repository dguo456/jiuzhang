...
"""
最简单的思路是将两个数组合并，然后返回新数组的中位数。两个有序数组的合并也是经典归并排序算法的一步，
我们可以新开一个数组，保存合并后的结果。但是我们这样会做额外的工作，因为我们不必保存整个新数组，只需要知道新数组的中位数即可。
...
# Method.2      归并写法2  参考Merge two sorted arrays模板
class Solution:
    """
    @param: A: An integer array
    @param: B: An integer array
...
"""
解题思路:
题目要求时间复杂度为O(log(m+n))，不难想到二分法。双指针方法中，我们一个一个的排除不可能的元素。
如果充分利用数组的有序性，就能一半一半的排除。具体来说，假设我们要找第k小数，通过二分，可以每次循环排除掉k/2个数。

算法流程:
建立辅助函数getKth，参数有数组A，A的起始指针start1和终止指针end1, 相对应的有B、start2和end2，以及整数k，
目标是找到A[start1:end1]和B[start2:end2]中第k小的元素。在主程序中，看m + n的奇偶性，并调用getKth函数。
如果是奇数，返回数组A和B的第(m + n) // 2 + 1小元素；如果是偶数，返回数组A和B的第(m + n) // 2小和第(m + n) // 2 + 1小元素的均值。
getKth(nums1, start1, end1, nums2, start2, end2, k)函数的实现方法:
1.如果有数组在[start:end]范围内为空，说明该数组已经排除完毕，第k小的元素一定存在于另一数组中，计算好索引位置返回即可。
2.如果k为1，说明已经找到第k小的数，那就是A[start1]和B[start2]中的较小值，直接返回即可。
...
k应该更新为k - (j - start2 + 1)。反之，说明A[start1:i]不可能为第k小元素。
我们对A[i + 1:end1]和B[start2:end2]继续送入getKth进行递归，k应该更新为k - (i - start1 + 1)。

复杂度
时间复杂度: O(log(m+n))。m和n分别是两个数组的长度。二分法的复杂度为O(log(m+n))。
空间复杂度: O(1)
"""
class Solution:
    """
...
"""
解题思路:
对于长度为m的数组A，我们把A划分成两个部分A1 = A[0, i-1]和A2 = A[i, m-1]。
对于长度为n的数组B，将B划分成B1 = B[0, j-1]和B2 = B[j, n-1]，使得len(A1) + len(B1) == len(A2) + len(B2)（记作条件1，
那么当A划分后，B的划分位置就是确定的。如果我们能够确定max(A1[:], B1[:]) <= min(A2[:], B2[:])（记作条件2，
说明我们已经找到合适的划分，能够把{A, B}分成长度相等的两份，且一份中的元素全部大于等于另一份。
那么，中位数就为(max(A1[:], B1[:]) + min(A2[:], B2[:])) / 2。怎么找到满足条件2的划分呢？
选择较短的数组，假设长度为m，对它可能的划分位置有m + 1种。我们可以进行二分搜索，那么时间复杂度能够进一步优化到O(log(min(m,n))

算法流程:
如果A长度大于B，两者交换一下，保证A是更短的。
对A进行二分，low和high初始化为0和m，每次循环不断缩小二分区间
对A的划分位置partition_x为区间中点low + (high - low) // 2，根据条件1计算出B的划分位置partition_y。
我们在划分处的两端，可以得到四个值: A左部分的最大值max_left_x，A右部分的最小值min_right_x，
B左部分的最大值max_left_y，B右部分的最小值min_right_y。如果某个值不存在，对于这种边界情况，
我们把最大值设为无穷小，最小值设为无穷大，保证后一步的比较恒成立。
如果此刻的划分满足了条件2，用上述四值来翻译一下就是max_left_x <= min_right_y and max_left_y <= min_right_x:，
那么我们就找到了中位数，是max(max_left_x, max_left_y) + min(min_right_x, min_right_y)) / 2。
如果不满足，如果max_left_x > min_right_y，说明partition_x位置靠右了，令high = partition_x - 1；
反之，说明partition_x位置靠左了，令low = partition_x + 1。继续我们的循环。

复杂度分析:
时间复杂度: O(log(min(m,n))。m和n分别是两个数组的长度。
空间复杂度: O(1)。
"""
class Solution:
    """
