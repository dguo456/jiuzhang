...
class Matris:
    ...
    def draw_surface(self):
        with_tetromino = self.blend(matrix=self.matrix)
    ...
    def request_rotation(self):
        ...
        # check if it can rotate and won't be blocked by wall
    ...
    def blend(self, shape=None, position=None, matrix=None):
        """
        ...
        for x in range(posX, posX+len(shape)):
            for y in range(posY, posY+len(shape)):
                if (copy.get((y, x), False) is False and shape[y-posY][x-posX] # shape is outside the matrix
                    or # coordinate is occupied by something else
                    ...
                    copy[(y,x)] = ('block', self.tetromino_block)
